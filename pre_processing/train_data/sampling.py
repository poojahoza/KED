...
'''
def _retrieve_trec_y3_section_ranking_samples(autograder_file_path: str,
                                      qrels_data: Dict) -> Dict:
    
    samples = dict()
    
    with lzma.open(autograder_file_path, 'rt', encoding='utf-8') as reader:
        for line in reader:
            data = json.loads(line)
            paragraphs_data = data[1]
                            ...
                            if rate['question_id'].startswith("tqa2:"):
                                ...
                                if query_id in qrels_data:
                                    if query_id in samples:
                                        temp_data = samples[query_id]
                                        if paragraph_id in qrels_data[query_id]:
                                            if paragraph_id not in temp_data['positive']:
                                                positive = temp_data['positive']
                                                positive.append(paragraph_id)
                                                pos_rel = temp_data['positive_rel']
                                                ...
                                                #         pos_rel.append([qrels_data[query_id][paragraph_id]/5, (1/ranking['rank'])])
                                                #         counter = 1
                                                
                                                # if counter == 0:
                                                #     pos_rel.append([qrels_data[query_id][paragraph_id]/5, 0])
                                                
                                                rankings = _get_multiple_ranking_data(item, query_id)
                                                pos_rel.append([qrels_data[query_id][paragraph_id]/5].extend(rankings))
                                                
                                                temp_data['positive'] = positive
                                                temp_data['positive_rel'] = pos_rel
                                        else:
                                            if paragraph_id not in temp_data['negative']:
                                                negative = temp_data['negative']
                                                negative.append(paragraph_id)
                                                neg_rel = temp_data['negative_rel']
                                                
                                                # counter = 0
                                                
                                                # for ranking in item['paragraph_data']['rankings']:
                                                #     if ranking['method'] == TREC_RANKING_METHOD and ranking['queryId'] == query_id:
                                                #         neg_rel.append([0, (1/ranking['rank'])])
                                                #         counter = 1
                                                ...
                                                rankings = _get_multiple_ranking_data(item, query_id)
                                                neg_rel.append([0].extend(rankings))
                                                
                                                temp_data['negative'] = negative
                                                temp_data['negative_rel'] = neg_rel
                                        samples[query_id] = temp_data
                                    else:
                                        temp_data = dict()
                                        temp_data['positive'] = []
